-- A GUI front-end for the autobutcher-breeder plugin.
--[====[

gui/autobutcher-breeder
=======================
An in-game interface for `autobutcher-breeder`. This script must be called
...
local plugin = require 'plugins.autobutcher-breeder'

WatchList = defclass(WatchList, gui.FramedScreen)

WatchList.ATTRS {
    frame_title = 'Autobutcher Breeder Watchlist',
...
function getSleepTimer()
    return plugin.autobutcher_breeder_getSleep()
end

function setSleepTimer(ticks)
    plugin.autobutcher_breeder_setSleep(ticks)
end

local function addCurrentRaces()
    local known_races = {}
    for _, entry in ipairs(plugin.autobutcher_breeder_getWatchList()) do
        known_races[entry.id] = true
    end

    local settings = plugin.autobutcher_breeder_getSettings()
    for _, unit in ipairs(df.global.world.units.all) do
        if not known_races[unit.race]
            ...
            plugin.autobutcher_breeder_setWatchListRace(
    ...
    self.subviews.bottom_ui:setText(
        {
            ...
            { key = 'CUSTOM_SHIFT_A', text = ': '..nextBreederState()..' Breeder',
              on_activate = self:callback('onToggleBreeder') }, '. ',
...
function WatchList:initListChoices()
    ...
    local watchlist = plugin.autobutcher_breeder_getWatchList()
    ...
    dlg.showInputPrompt(
        ...
        function(text)
            ...
            if self:checkUserInput(count, text) then
                fk = count
                if selidx == 1 then
                    plugin.autobutcher_breeder_setDefaultTargetAll( fk, mk, fa, ma )
                end
                if selidx == 2 then
                    plugin.autobutcher_breeder_setDefaultTargetNew( fk, mk, fa, ma )
                end
                if selidx > 2 then
                    plugin.autobutcher_breeder_setWatchListRace(id, fk, mk, fa, ma, watched)
                end
                self:initListChoices()
            end
        end
...
function WatchList:onToggleWatching()
    ...
    if selidx > 2 then
        local entry = selobj.obj
        plugin.autobutcher_breeder_setWatchListRace(entry.id, entry.fk, entry.mk, entry.fa, entry.ma, not entry.watched)
    end
    self:initListChoices()
end
...
function WatchList:onDeleteEntry()
    ...
    dlg.showYesNoPrompt(
        ...
        COLOR_YELLOW,
        function()
            plugin.autobutcher_breeder_removeFromWatchList(selobj.obj.id)
            self:initListChoices()
        end
    )
end
...
function WatchList:onUnbutcherRace()
    ...
    if selidx > 2 then
        ...
        plugin.autobutcher_breeder_unbutcherRace(entry.id)
        self:initListChoices()
        self:updateBottom()
    end
end
...
function WatchList:onButcherRace()
    ...
    if selidx > 2 then
        local entry = selobj.obj
        local race = entry.name
        plugin.autobutcher_breeder_butcherRace(entry.id)
        self:initListChoices()
        self:updateBottom()
    end
end
...
function WatchList:onSetRow()
    ...
    dlg.showInputPrompt(
        ...
        function(text)
            ...
            if self:checkUserInput(count, text) then
                if selidx == 1 then
                    plugin.autobutcher_breeder_setDefaultTargetAll( count, count, count, count )
                end
                if selidx == 2 then
                    plugin.autobutcher_breeder_setDefaultTargetNew( count, count, count, count )
                end
                if selidx > 2 then
                    plugin.autobutcher_breeder_setWatchListRace(id, count, count, count, count, watched)
                end
                self:initListChoices()
            end
        end
...
function WatchList:onToggleBreeder()
    if(plugin.isEnabled()) then
