...
for assignment_idx,assignment in ipairs(my_entity.positions.assignments) do
    if assignment.position_id==monarch_id then
        ...
        for k,v in pairs(oldfig.entity_links) do
            if df.histfig_entity_link_positionst:is_instance(v) and v.assignment_id==assignment.id and v.entity_id==df.global.ui.civ_id then --hint:df.histfig_entity_link_positionst
